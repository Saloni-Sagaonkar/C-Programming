...
int main(){
	...
	scanf("%lf", &principal_amount);  			//1000.0
	
	// Notice %lf is the format specifier for double

	total_interest = (principal_amount * duration * rate);

	printf("Your total payable interest is $ %lf  when principal_amount given as %0.1lf, duration is %d", total_interest, principal_amount, duration);
	//Your total payable interest is $ 300.000000  when principal_amount given as 1000.0, duration is 5
	return 0;
}
